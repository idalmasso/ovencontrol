<template>
    ...
    <v-container>
        <h2>Server</h2>    
        <v-text-field label="Temperatura rampa di test" 
            v-model="configData.config.server['test-ramp-temperature']"
            type="number">
        </v-text-field>
        <v-text-field label="Tempo minuti rampa di test" 
...
function saveData(){
    fetch("http://localhost:3333/api/configuration/oven-config", {method:"POST", body:JSON.stringify(configData.config)}).then((response) => {
    if (response.ok) {
        router.back()
    }
    })
...
onMounted(()=>{
    fetch("http://localhost:3333/api/configuration/oven-config").then((response) => {
    if (response.ok) {
